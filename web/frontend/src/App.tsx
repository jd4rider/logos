...
export default function App() {
  ...
  const selectBible = useCallback(async (bible: Bible) => {
    ...
    setLoading(true);
    try {
      const b = await api.getBooks(bible.id);
      setBooks(b);
      setView('books');
    } catch (e: unknown) {
  ...
  const selectBook = useCallback(async (book: Book) => {
    ...
    setLoading(true);
    try {
      const chs = await api.getChapters(currentBible.id, book.id);
      setChapters(chs);
      setView('chapters');
    } catch (e: unknown) {
      setError(e instanceof Error ? e.message : String(e));
  ...
  const selectChapter = useCallback(async (chapter: Chapter) => {
    ...
    setLoading(true);
    try {
      const ch = await api.getChapter(currentBible.id, chapter.id);
  ...
  const loadChapterById = useCallback(async (chapterId: string) => {
    ...
    setLoading(true);
    try {
      const ch = await api.getChapter(currentBible.id, chapterId);
      setCurrentChapter(ch);
      setView('reader');
    } catch (e: unknown) {
      setError(e instanceof Error ? e.message : String(e));
  ...
  const handleSearch = useCallback(async (query: string) => {
    ...
    setLoading(true);
    try {
      const data = await api.search(currentBible.id, query);
    ...
    <div className="flex flex-col h-screen bg-bg text-text">
      ...
      <header className="flex items-center justify-between px-4 py-2 bg-surface border-b border-border">
        ...
        <div className="flex items-center gap-2">
          {ttsEngine && ttsEngine !== 'none' && (
            <span className="text-xs text-muted">TTS: {ttsEngine}</span>
          )}
          {(view === 'reader' || view === 'books' || view === 'chapters') && (
            <button
          ...
          {view !== 'bibles' && (
            <button
              onClick={goBack}
              className="px-3 py-1 text-sm bg-highlight text-muted border border-border rounded hover:text-text transition-colors"
            >
              ← Back
            </button>
          )}
