...
type biblesLoadedMsg   struct{ bibles []api.Bible; offline bool; offlineFlags []bool }
...
func (i voiceItem) Description() string {
	...
	case "piper":  return "Piper TTS — high quality neural"
	case "kokoro": return "Kokoro TTS — natural neural"
	case "say":    return "macOS built-in"
	}
	return ""
}
...
Language:     api.Language{Name: t.Language},
},
offline: true,
})
}
...
for i, b := range msg.books { items[i] = bookItem{b} }
...
for i, c := range msg.chapters { items[i] = chapterItem{c} }
...
for i, v := range msg.results.Verses { items[i] = searchItem{v} }
...
case struct{ text string; err error }:
if m.aiPanel != nil {
if msg.err != nil {
...
if m.state == StateReader { m.viewport.LineUp(3) }
case tea.MouseButtonWheelDown:
if m.state == StateReader { m.viewport.LineDown(3) }
}
return m.updateActiveComponent(msg)
}
...
m.loading = true; m.state = StateLoading
...
case "esc", "backspace": m.state = StateBooks
case "/":                return m.openSearch()
}
...
case "g": m.viewport.GotoTop()
case "G": m.viewport.GotoBottom()
case "/": return m.openSearch()
case "a": return m.openAIPanel()
}
...
case "esc", "backspace":
if m.currentChapter.ID != "" { m.state = StateReader } else { m.state = StateBooks }
case "/": return m.openSearch()
}
...
if m.tts != nil { m.tts.SetVoiceEntry(item.entry) }
m.state = m.prevState
}
}
...
for i, v := range voices { items[i] = voiceItem{v} }
...
if v.ID == cur.ID { m.voiceList.Select(idx); break }
}
}
m.prevState = m.state
...
var cmd tea.Cmd
switch m.state {
case StateBibles:       m.bibleList,   cmd = m.bibleList.Update(msg)
case StateBooks:        m.bookList,    cmd = m.bookList.Update(msg)
case StateChapters:     m.chapterList, cmd = m.chapterList.Update(msg)
case StateReader:       m.viewport,    cmd = m.viewport.Update(msg)
case StateSearch:       m.searchList,  cmd = m.searchList.Update(msg)
case StateVoicePicker:  m.voiceList,   cmd = m.voiceList.Update(msg)
...
var crumbs []string
if m.selectedBible.Abbreviation != "" { crumbs = append(crumbs, stripEngPrefix(m.selectedBible.Abbreviation)) }
if m.selectedBook.Name != ""           { crumbs = append(crumbs, m.selectedBook.Name) }
...
var hints string
switch m.state {
case StateLoading:      hints = "loading…"
case StateBibles:       hints = "↑↓ navigate  •  enter select  •  i import  •  / search  •  q quit"
case StateBooks:        hints = "↑↓ navigate  •  enter select  •  esc back  •  / search  •  q quit"
case StateChapters:     hints = "↑↓ navigate  •  enter select  •  esc back  •  / search  •  q quit"
case StateReader:       hints = "↑↓ scroll  •  s speak  •  S stop  •  a AI  •  v voice  •  n/p ch  •  / search  •  esc back  •  q quit"
case StateSearch:       hints = "↑↓ navigate  •  enter open  •  / new search  •  esc back  •  q quit"
case StateVoicePicker:  hints = "↑↓ navigate  •  enter select voice  •  esc cancel"
case StateImport:
if m.importPanel != nil { hints = m.importPanel.Hints() }
case StateAI:
if m.aiPanel != nil { hints = m.aiPanel.Hints() }
}
